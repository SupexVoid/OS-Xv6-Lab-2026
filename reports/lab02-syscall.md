...
- 正式答辩环境计划：WSL 2 + Ubuntu，与 MIT 推荐环境保持一致。

由于新版多架构工具链不会自动选择 xv6 所需的 64 位 ABI，Makefile 显式加入 `-march=rv64gc -mabi=lp64` 和 `elf64lriscv`。Windows 宿主机生成文件系统镜像时还必须使用二进制模式，因此对 `mkfs` 增加了兼容处理。这些修改只影响构建环境，不改变实验逻辑。
...
学长仓库用于核对实验范围和涉及文件，本分支从 MIT 官方 `syscall` starter 独立实现。核对后额外加强了三个边界：
...
### 1. 自动评分的一次 Windows 管道超时

第一次完整评分中，`trace all grep` 在 xv6 已显示命令后没有被临时 Windows 评分适配器继续读取，因而超时。相同命令在手动 QEMU 会话中输出正确，单项复跑也通过；随后再次完整运行，全部评分项通过。由此确认是宿主机管道轮询的偶发抖动，不是内核实现的竞态。
...
MIT `grade-lab-syscall` 的测试内容保持不变，仅使用未提交的 Windows 进程与管道兼容层启动 QEMU。最终结果为：
...
本实验把“用户函数调用”与“受控进入内核”连接起来：系统调用不是普通函数跳转，而是通过寄存器、陷阱和分发表进入内核。`trace` 说明进程控制块保存了跨系统调用的进程级状态；`sysinfo` 则说明读取全局状态也必须遵守并发锁规则，并通过 `copyout()` 跨越用户/内核边界。答辩时应能画出调用链，解释掩码为何在 `fork` 中继承，以及为什么不能直接写用户指针。
