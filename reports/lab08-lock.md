...
- 交叉编译器：xPack RISC-V GCC 15.2.0，目标前缀 `riscv-none-elf-`；
- 虚拟机：QEMU 9.2.0，`virt` 机器、3 个 hart、128 MiB 内存；
- 主机：Windows 11；正式答辩环境计划使用 WSL 2 + Ubuntu 复现 MIT 原始命令。
...
批量偷页是慢路径频率与临界区长度的折中。初版每次只偷 64 页，正确性和争用测试都通过，但 Windows TCG 下 `kalloctest` 的 50 轮全内存遍历超过 200 秒。改为有上限的 1024 页后，仍不长期垄断整个 donor 链表，完整测试在 182.6 秒内结束。
...
学长仓库用于确认 per-CPU freelist、批量偷页和 hash bucket 的总体方向。本实现进一步加强了：
...
### 3. Windows GDB 评分包装停滞

临时 Windows 评分适配器使用 `-S/-gdb` 启动时，锁实验在 QEMU 内运行显著变慢，并在评分器清理子进程时停滞；同一内核、同一 CPU/内存参数直接串口运行能稳定完成。最终保留 MIT 原测试程序和超时标准，绕过临时 GDB 包装逐项运行并按原评分正则核对。课程答辩使用 WSL/Linux 时可直接执行 `make grade`。
...
减少锁争用不是简单地“多加几把锁”，而是让锁的粒度与数据分区一致，并重新定义受保护的不变量。答辩时应能解释：为什么 `cpuid()` 需要关中断、为什么偷页不能同时持有两把 freelist 锁、为什么 cache miss 必须二次查找、无锁扫描后为何要复查 `refcnt`、固定桶锁顺序如何避免死锁，以及“全部桶锁”虽然容易保证正确却会破坏并行度。
