...
Makefile 和 `mkfs` 继续使用前述 RV64 与 Windows 二进制模式兼容修改，这些修改不改变实验逻辑。
...
完整回答保存在 `answers-traps.txt`。汇编地址会随编译器版本和最终链接内容变化，因此答辩时应根据当前 `call.asm` 解释，而不是死记学长仓库中的旧地址。
...
最初为 `sigalarm` 增加了常见的空指针检查，结果 test0 和 test1 始终不触发，而使用 `slow_handler` 的 test2 可以触发。计时复现表明前两组循环分别运行数秒和约 30 秒，不是循环过快。
...
MIT `grade-lab-traps` 测试内容保持不变，仅使用未提交的 Windows 进程、管道和 `addr2line` 名称兼容层。结果为：
...
alarm 本质上是一个简化的用户态信号机制：内核把正常执行现场保存起来，伪造一次到用户 handler 的返回，再由特殊系统调用恢复原现场。它连接了汇编调用约定、陷阱帧、系统调用返回值和进程状态。答辩时应能解释 `epc`、`ra`、`sp`、`s0`、`a0` 的作用，说明为何必须保存整个 trapframe、为什么禁止 handler 重入，以及为什么 xv6 的用户地址 0 可以是合法函数地址。
