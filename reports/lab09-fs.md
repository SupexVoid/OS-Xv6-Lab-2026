...
- 虚拟机：QEMU `virt`、1 个 hart、128 MiB 内存；
...
`bigfile` 需要写入并读回 65,803 个数据块，首次运行耗时较长。先执行 `make clean` 重新生成与新 inode 布局一致的镜像，并确保项目位于 WSL 的 Linux 文件系统中，再运行完整测试；最终在 139.8 秒内完成写入和读回，低于实验限制。
...
双重间接块本质上是把文件逻辑块号拆成两级索引；符号链接则把“目录项定位到 inode”和“open 是否继续解释 inode 内容”分成两个阶段。实现过程表明，扩大文件容量不仅要增加分配路径，还必须保证 `itrunc` 对称释放；路径跟随也必须在每一跳正确交接 inode 锁和引用。
